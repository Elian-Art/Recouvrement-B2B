...
### Phase 4 — Intégrations & Polish (complétée ✅)
- [x] **Step 1** — Migration DB : `email_templates`, `integrations`, champs `locale` / `stripe_subscription_id` / `onboarding_completed` sur `organizations`
- [x] **Step 2** — Import CSV/Excel : `/invoices/import` wizard 4 étapes (upload → mapping → preview → done) + `/api/invoices/import`
- [x] **Step 3** — Templates email/SMS : `/settings/templates` CRUD (upsert par `org_id,key,channel`) + variables `{{contact_name}}` etc.
- [x] **Step 4** — SaaS Billing : `/settings/billing` (4 plans) + `/api/billing/portal` (Stripe Customer Portal)
- [x] **Step 5** — Intégrations QuickBooks + Xero : `/settings/integrations` + OAuth connect/callback/disconnect/sync pour les deux providers
- [x] **Step 6** — i18n : next-intl v4, `NextIntlClientProvider` dans le layout dashboard, `src/messages/fr.json` + `src/messages/en.json`
- [x] **Step 7** — Onboarding wizard : `/onboarding` (multi-step org→débiteur→facture→scenario) + routes `/api/onboarding/*`
- [x] **Step 8** — Sidebar mis à jour avec tous les nouveaux liens
...
### Phase 4 — Intégrations & Polish (complétée)
- **Packages ajoutés** : `papaparse`, `@types/papaparse`, `xlsx`, `next-intl`
- **CSV/Excel import** : `src/app/(dashboard)/invoices/import/_components/import-wizard.tsx` — wizard client 4 étapes, `guessMapping()` pour détection auto des colonnes, `validateRow()`. `/api/invoices/import` upserte debtors + invoices, parse dates ISO/FR/US.
- **Templates email/SMS** : table `email_templates (org_id, key, channel)` avec unique constraint. Actions server dans `src/app/(dashboard)/settings/templates/actions.ts`. Variables supportées : `{{contact_name}}`, `{{invoice_number}}`, `{{amount}}`, `{{due_date}}`, `{{payment_link}}`, `{{org_name}}`.
- **SaaS Billing** : `/api/billing/portal` crée le customer Stripe si absent, puis ouvre le portail. Plans définis statiquement dans `billing/page.tsx` (free/starter/pro/enterprise).
- **QuickBooks OAuth** : flow standard OAuth2 vers `https://appcenter.intuit.com/connect/oauth2`. Tokens stockés dans `integrations`. Sync via l'API REST QB v3 (`/query`). Token refresh automatique dans les routes sync.
- **Xero OAuth** : flow vers `https://login.xero.com/identity/connect/authorize`. Récupération du `tenantId` via `/connections`. Sync via `https://api.xero.com/api.xro/2.0/Invoices`. Dates Xero au format `/Date(timestamp)/`.
- **i18n** : sans routing URL — `NextIntlClientProvider` wrappé dans `I18nProvider` (`src/lib/i18n/provider.tsx`). Locale lue depuis `organizations.locale`. Messages dans `src/messages/fr.json` et `en.json`.
- **Onboarding wizard** : affiché quand `organizations.onboarding_completed = false`. Steps : 1=org name, 2=premier débiteur, 3=première facture, 4=scenario confirmé. Marqué complet via `POST /api/onboarding/complete`.
- **DB ajouts** : `organizations.locale` (default 'fr'), `organizations.onboarding_completed` (default false), `organizations.stripe_subscription_id`.
